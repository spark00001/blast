                ...
                if (preg_match('~^http/(\d+\.?\d*)\s+(\d+)~iUs', $line, $matches)) {
                    ...
                    switch (preg_replace('~\s~', '', strtoupper($pair[0]))) {
                        ...
                        case 'SET-COOKIE': {
                            ...
                            if (is_array($cookie) && count($cookie) > 1) $cookie[1] = preg_replace('~expires\s*=\s*~iUs', '', $cookie[1]);
                            else $cookie[1] = '';
                        ...
                        case 'WWW-AUTHENTICATE': {
                            ...
                            $head['WWW_AUTHENTICATE_REALM'] = isset($m[2]) ? $m[2] : '';
                        }
                            break;
                        ...
                        case 'REFRESH': {
                            $m = explode(';', $pair[1]);
                            $head['HTTP_REFRESH'] = Array((int)$m[0], isset($m[1]) ? preg_replace('~^\surl\s*=(.)$~iUs', '$1', $m[1]) : '');
                        }
                            break;
                        case 'CONTENT-TYPE': {
                            $this->doctype = $pair[1];
